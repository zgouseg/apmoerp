        ...
        <div class="flex h-14 items-center justify-between gap-3">
            ...
            <div class="flex items-center gap-2">
                ...
                @hasSection('page-title')
                    <h1 class="text-sm sm:text-base font-semibold text-slate-800 dark:text-slate-200">
                        @yield('page-title')
                    </h1>
                @else
                    <h1 class="text-sm sm:text-base font-semibold text-slate-800 dark:text-slate-200">
            ...
            <div class="flex items-center gap-2">
                ...
                <div class="relative" x-data="{ open: false }" @click.away="open = false">
                    <button type="button" @click="open = !open"
                            class="inline-flex items-center gap-2 rounded-full border border-emerald-100 dark:border-slate-700 bg-white dark:bg-slate-800 px-2.5 py-1.5 text-xs sm:text-sm text-slate-700 dark:text-slate-200 shadow-sm shadow-emerald-500/20 hover:bg-emerald-50 dark:hover:bg-slate-700">
                    ...
                    <div x-show="open" 
                         ...
                         class="absolute {{ app()->getLocale() === 'ar' ? 'left-0' : 'right-0' }} mt-2 w-56 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 shadow-lg z-popover"
                         style="display: none;">
                        <div class="p-3 border-b border-slate-100 dark:border-slate-700">
                            <p class="text-sm font-medium text-slate-800 dark:text-slate-200">{{ auth()->user()->name ?? __('User') }}</p>
                            <p class="text-xs text-slate-500 dark:text-slate-400">{{ auth()->user()->email ?? '' }}</p>
                        </div>
                        <div class="p-2 space-y-1">
                            <a href="{{ route('profile.edit') }}" class="flex items-center gap-2 px-3 py-2 text-sm text-slate-700 dark:text-slate-300 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-700">
                                <svg class="w-4 h-4 text-slate-500 dark:text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/>
                                </svg>
                                {{ __('My Profile') }}
                            </a>
                            <a href="{{ route('admin.settings') }}" class="flex items-center gap-2 px-3 py-2 text-sm text-slate-700 dark:text-slate-300 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-700">
                                <svg class="w-4 h-4 text-slate-500 dark:text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div class="p-2 border-t border-slate-100 dark:border-slate-700">
                            <form method="POST" action="{{ route('logout') }}">
                                @csrf
                                <button type="submit" class="flex items-center gap-2 w-full px-3 py-2 text-sm text-red-600 dark:text-red-400 rounded-lg hover:bg-red-50 dark:hover:bg-red-900/30">
