{{-- resources/views/livewire/rental/tenants/form.blade.php --}}
<div class="space-y-4">
    <div class="flex items-center justify-between gap-2">
        <div>
            <h1 class="text-lg font-semibold text-slate-800 dark:text-slate-100">
                {{ $tenantId ? __('Edit Tenant') : __('Add Tenant') }}
            </h1>
            <p class="text-sm text-slate-500 dark:text-slate-400">
                {{ __('Manage rental tenant information.') }}
            </p>
        </div>
    </div>
        ...
        <div class="flex justify-end gap-3">
            ...
            <button type="submit" class="erp-btn-primary" wire:loading.attr="disabled">
                <span wire:loading.remove wire:target="save">
                    {{ $tenantId ? __('Update Tenant') : __('Create Tenant') }}
                </span>
