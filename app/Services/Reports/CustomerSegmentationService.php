 ...
 *
 * SECURITY (V40-SQL-09): SQL Expression Safety - REVIEWED
 * ========================================================
 ...
 *
 * 1. $datediffExpr: Generated by DatabaseCompatibilityService::daysDifference()
 *    - Validates all parameters via regex and whitelist
 *    - Returns hardcoded SQL expressions for each database driver
 *
 * 2. Column names (sales.sale_date, customers.id) are hardcoded in codebase
 *
 * @security-reviewed V40 - SQL expressions validated via DatabaseCompatibilityService
 */
class CustomerSegmentationService
{
    private DatabaseCompatibilityService $dbCompat;

    public function __construct()
    {
        $this->dbCompat = app(DatabaseCompatibilityService::class);
    }
     ...
     *
     * SECURITY NOTE: The $datediffExpr is generated by DatabaseCompatibilityService::daysDifference()
    ...
    {
        // V37-SEC: Use DatabaseCompatibilityService for validated date expressions
        // SECURITY: DatabaseCompatibilityService returns only hardcoded expressions
        // @security-reviewed V43 - $datediffExpr is validated by DatabaseCompatibilityService regex patterns
        $datediffExpr = $this->dbCompat->daysDifference($this->dbCompat->now(), 'MAX(sales.sale_date)');

        // @phpstan-ignore-next-line - $datediffExpr is regex-validated by DatabaseCompatibilityService
     ...
     * No user input is interpolated into the SQL.
     *
     * @security-reviewed V43 - $datediffExpr is validated by DatabaseCompatibilityService
     */
